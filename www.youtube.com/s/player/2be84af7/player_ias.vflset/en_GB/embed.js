(function (g) {
  var window = this; /*
 SPDX-License-Identifier: Apache-2.0 */
  /*

 Copyright Google LLC All Rights Reserved.

 Use of this source code is governed by an MIT-style license that can be
 found in the LICENSE file at https://angular.io/license
*/
  /*

 Copyright 2017 Google LLC
 SPDX-License-Identifier: BSD-3-Clause
*/
  ("use strict");
  var Wlb = function (a) {
      a.mutedAutoplay = !1;
      a.endSeconds = NaN;
      a.limitedPlaybackDurationInSeconds = NaN;
      g.iT(a);
    },
    Xlb = function () {
      return {
        I: "svg",
        W: {
          height: "100%",
          version: "1.1",
          viewBox: "0 0 110 26",
          width: "100%",
        },
        V: [
          {
            I: "path",
            Dc: !0,
            S: "ytp-svg-fill",
            W: {
              d: "M 16.68,.99 C 13.55,1.03 7.02,1.16 4.99,1.68 c -1.49,.4 -2.59,1.6 -2.99,3 -0.69,2.7 -0.68,8.31 -0.68,8.31 0,0 -0.01,5.61 .68,8.31 .39,1.5 1.59,2.6 2.99,3 2.69,.7 13.40,.68 13.40,.68 0,0 10.70,.01 13.40,-0.68 1.5,-0.4 2.59,-1.6 2.99,-3 .69,-2.7 .68,-8.31 .68,-8.31 0,0 .11,-5.61 -0.68,-8.31 -0.4,-1.5 -1.59,-2.6 -2.99,-3 C 29.11,.98 18.40,.99 18.40,.99 c 0,0 -0.67,-0.01 -1.71,0 z m 72.21,.90 0,21.28 2.78,0 .31,-1.37 .09,0 c .3,.5 .71,.88 1.21,1.18 .5,.3 1.08,.40 1.68,.40 1.1,0 1.99,-0.49 2.49,-1.59 .5,-1.1 .81,-2.70 .81,-4.90 l 0,-2.40 c 0,-1.6 -0.11,-2.90 -0.31,-3.90 -0.2,-0.89 -0.5,-1.59 -1,-2.09 -0.5,-0.4 -1.10,-0.59 -1.90,-0.59 -0.59,0 -1.18,.19 -1.68,.49 -0.49,.3 -1.01,.80 -1.21,1.40 l 0,-7.90 -3.28,0 z m -49.99,.78 3.90,13.90 .18,6.71 3.31,0 0,-6.71 3.87,-13.90 -3.37,0 -1.40,6.31 c -0.4,1.89 -0.71,3.19 -0.81,3.99 l -0.09,0 c -0.2,-1.1 -0.51,-2.4 -0.81,-3.99 l -1.37,-6.31 -3.40,0 z m 29.59,0 0,2.71 3.40,0 0,17.90 3.28,0 0,-17.90 3.40,0 c 0,0 .00,-2.71 -0.09,-2.71 l -9.99,0 z m -53.49,5.12 8.90,5.18 -8.90,5.09 0,-10.28 z m 89.40,.09 c -1.7,0 -2.89,.59 -3.59,1.59 -0.69,.99 -0.99,2.60 -0.99,4.90 l 0,2.59 c 0,2.2 .30,3.90 .99,4.90 .7,1.1 1.8,1.59 3.5,1.59 1.4,0 2.38,-0.3 3.18,-1 .7,-0.7 1.09,-1.69 1.09,-3.09 l 0,-0.5 -2.90,-0.21 c 0,1 -0.08,1.6 -0.28,2 -0.1,.4 -0.5,.62 -1,.62 -0.3,0 -0.61,-0.11 -0.81,-0.31 -0.2,-0.3 -0.30,-0.59 -0.40,-1.09 -0.1,-0.5 -0.09,-1.21 -0.09,-2.21 l 0,-0.78 5.71,-0.09 0,-2.62 c 0,-1.6 -0.10,-2.78 -0.40,-3.68 -0.2,-0.89 -0.71,-1.59 -1.31,-1.99 -0.7,-0.4 -1.48,-0.59 -2.68,-0.59 z m -50.49,.09 c -1.09,0 -2.01,.18 -2.71,.68 -0.7,.4 -1.2,1.12 -1.49,2.12 -0.3,1 -0.5,2.27 -0.5,3.87 l 0,2.21 c 0,1.5 .10,2.78 .40,3.78 .2,.9 .70,1.62 1.40,2.12 .69,.5 1.71,.68 2.81,.78 1.19,0 2.08,-0.28 2.78,-0.68 .69,-0.4 1.09,-1.09 1.49,-2.09 .39,-1 .49,-2.30 .49,-3.90 l 0,-2.21 c 0,-1.6 -0.2,-2.87 -0.49,-3.87 -0.3,-0.89 -0.8,-1.62 -1.49,-2.12 -0.7,-0.5 -1.58,-0.68 -2.68,-0.68 z m 12.18,.09 0,11.90 c -0.1,.3 -0.29,.48 -0.59,.68 -0.2,.2 -0.51,.31 -0.81,.31 -0.3,0 -0.58,-0.10 -0.68,-0.40 -0.1,-0.3 -0.18,-0.70 -0.18,-1.40 l 0,-10.99 -3.40,0 0,11.21 c 0,1.4 .18,2.39 .68,3.09 .49,.7 1.21,1 2.21,1 1.4,0 2.48,-0.69 3.18,-2.09 l .09,0 .31,1.78 2.59,0 0,-14.99 c 0,0 -3.40,.00 -3.40,-0.09 z m 17.31,0 0,11.90 c -0.1,.3 -0.29,.48 -0.59,.68 -0.2,.2 -0.51,.31 -0.81,.31 -0.3,0 -0.58,-0.10 -0.68,-0.40 -0.1,-0.3 -0.21,-0.70 -0.21,-1.40 l 0,-10.99 -3.40,0 0,11.21 c 0,1.4 .21,2.39 .71,3.09 .5,.7 1.18,1 2.18,1 1.39,0 2.51,-0.69 3.21,-2.09 l .09,0 .28,1.78 2.62,0 0,-14.99 c 0,0 -3.40,.00 -3.40,-0.09 z m 20.90,2.09 c .4,0 .58,.11 .78,.31 .2,.3 .30,.59 .40,1.09 .1,.5 .09,1.21 .09,2.21 l 0,1.09 -2.5,0 0,-1.09 c 0,-1 -0.00,-1.71 .09,-2.21 0,-0.4 .11,-0.8 .31,-1 .2,-0.3 .51,-0.40 .81,-0.40 z m -50.49,.12 c .5,0 .8,.18 1,.68 .19,.5 .28,1.30 .28,2.40 l 0,4.68 c 0,1.1 -0.08,1.90 -0.28,2.40 -0.2,.5 -0.5,.68 -1,.68 -0.5,0 -0.79,-0.18 -0.99,-0.68 -0.2,-0.5 -0.31,-1.30 -0.31,-2.40 l 0,-4.68 c 0,-1.1 .11,-1.90 .31,-2.40 .2,-0.5 .49,-0.68 .99,-0.68 z m 39.68,.09 c .3,0 .61,.10 .81,.40 .2,.3 .27,.67 .37,1.37 .1,.6 .12,1.51 .12,2.71 l .09,1.90 c 0,1.1 .00,1.99 -0.09,2.59 -0.1,.6 -0.19,1.08 -0.49,1.28 -0.2,.3 -0.50,.40 -0.90,.40 -0.3,0 -0.51,-0.08 -0.81,-0.18 -0.2,-0.1 -0.39,-0.29 -0.59,-0.59 l 0,-8.5 c .1,-0.4 .29,-0.7 .59,-1 .3,-0.3 .60,-0.40 .90,-0.40 z",
            },
          },
        ],
      };
    },
    Ylb = function (a, b) {
      var c = g.Ka.apply(2, arguments),
        d;
      b = null != (d = b) ? d : {};
      return { type: a, props: b, children: c };
    },
    Zlb = function (a) {
      return a.children;
    },
    $lb = function (a) {
      var b = {},
        c;
      for (c in a)
        (b = { UQ: b.UQ, MP: b.MP }),
          (b.UQ = q5[c]),
          (b.MP = a[c]),
          (q5[c] = (function (d) {
            return function () {
              var e = g.Ka.apply(0, arguments);
              d.UQ.apply(null, g.qa(e));
              d.MP.apply(null, g.qa(e));
            };
          })(b));
    },
    amb = function () {},
    r5 = function (a, b) {
      for (; a.length > b; ) a.pop();
    },
    bmb = function (a) {
      a = Array(a);
      r5(a, 0);
      return a;
    },
    cmb = function (a, b, c) {
      if (null == c) a.removeAttribute(b);
      else {
        var d =
          0 === b.lastIndexOf("xml:", 0)
            ? "http://www.w3.org/XML/1998/namespace"
            : 0 === b.lastIndexOf("xlink:", 0)
            ? "http://www.w3.org/1999/xlink"
            : null;
        d ? a.setAttributeNS(d, b, c) : a.setAttribute(b, c);
      }
    },
    emb = function (a, b, c) {
      a = a.style;
      if ("string" === typeof c) a.cssText = c;
      else {
        a.cssText = "";
        for (var d in c)
          if (dmb.call(c, d)) {
            b = d;
            var e = c[d];
            0 <= b.indexOf("-") ? a.setProperty(b, e) : (a[b] = e);
          }
      }
    },
    fmb = function (a, b, c) {
      var d = typeof c;
      "object" === d || "function" === d ? (a[b] = c) : cmb(a, b, c);
    },
    gmb = function () {
      var a = new amb();
      a.__default = fmb;
      a.style = emb;
      return a;
    },
    hmb = function (a, b, c, d) {
      (d[b] || d.__default)(a, b, c);
    },
    imb = function (a) {
      this.created = [];
      this.j = [];
      this.node = a;
    },
    jmb = function (a, b) {
      this.j = null;
      this.B = a;
      this.key = b;
      this.text = void 0;
    },
    kmb = function (a, b, c) {
      b = new jmb(b, c);
      return (a.__incrementalDOMData = b);
    },
    s5 = function (a, b) {
      if (a.__incrementalDOMData) return a.__incrementalDOMData;
      var c = 1 === a.nodeType ? a.localName : a.nodeName,
        d = 1 === a.nodeType ? a.getAttribute("key") : null;
      b = kmb(a, c, 1 === a.nodeType ? d || b : null);
      if (1 === a.nodeType && ((a = a.attributes), (c = a.length))) {
        d = b.j || (b.j = bmb(2 * c));
        for (var e = 0, f = 0; e < c; e += 1, f += 2) {
          var h = a[e],
            l = h.value;
          d[f] = h.name;
          d[f + 1] = l;
        }
      }
      return b;
    },
    lmb = function (a, b, c, d, e) {
      return b == c && d == e;
    },
    w5 = function (a) {
      for (var b = t5, c = u5(); c !== a; ) {
        var d = c.nextSibling;
        b.removeChild(c);
        v5.j.push(c);
        c = d;
      }
    },
    u5 = function () {
      return x5 ? x5.nextSibling : t5.firstChild;
    },
    mmb = function (a, b) {
      x5 = u5();
      var c;
      a: {
        if ((c = x5)) {
          do {
            var d = c,
              e = a,
              f = b,
              h = s5(d, f);
            if (y5(d, e, h.B, f, h.key)) break a;
          } while (b && (c = c.nextSibling));
        }
        c = null;
      }
      c ||
        ("#text" === a
          ? ((a = z5.createTextNode("")), kmb(a, "#text", null))
          : ((c = z5),
            (d = t5),
            "function" === typeof a
              ? (c = new a())
              : (c = (d =
                  "svg" === a
                    ? "http://www.w3.org/2000/svg"
                    : "math" === a
                    ? "http://www.w3.org/1998/Math/MathML"
                    : null == d || "foreignObject" === s5(d).B
                    ? null
                    : d.namespaceURI)
                  ? c.createElementNS(d, a)
                  : c.createElement(a)),
            kmb(c, a, b),
            (a = c)),
        v5.created.push(a),
        (c = a));
      a = c;
      if (a !== x5) {
        if (0 <= A5.indexOf(a))
          for (b = t5, c = a.nextSibling, d = x5; null !== d && d !== a; )
            (e = d.nextSibling), b.insertBefore(d, c), (d = e);
        else t5.insertBefore(a, x5);
        x5 = a;
      }
    },
    nmb = function (a, b) {
      mmb(a, b);
      t5 = x5;
      x5 = null;
      return t5;
    },
    pmb = function (a, b) {
      b = void 0 === b ? {} : b;
      var c = void 0 === b.matches ? lmb : b.matches;
      return function (d, e, f) {
        var h = v5,
          l = z5,
          m = A5,
          n = B5,
          p = x5,
          q = t5,
          r = y5;
        z5 = d.ownerDocument;
        v5 = new imb(d);
        y5 = c;
        B5 = [];
        x5 = null;
        var t = (t5 = d.parentNode),
          u,
          x = omb.call(d);
        if (
          (u =
            11 === x.nodeType || 9 === x.nodeType ? x.activeElement : null) &&
          d.contains(u)
        ) {
          for (x = []; u !== t; )
            x.push(u), (u = u.parentNode || (t ? u.host : null));
          t = x;
        } else t = [];
        A5 = t;
        try {
          return a(d, e, f);
        } finally {
          (d = v5),
            C5 && 0 < d.j.length && C5(d.j),
            (z5 = l),
            (v5 = h),
            (y5 = r),
            (B5 = n),
            (x5 = p),
            (t5 = q),
            (A5 = m);
        }
      };
    },
    qmb = function (a, b, c, d) {
      D5.push(hmb);
      D5.push(a);
      D5.push(b);
      D5.push(c);
      D5.push(d);
    },
    rmb = function (a) {
      mmb("#text", null);
      var b = x5;
      var c = s5(b);
      if (c.text !== a) {
        c = c.text = a;
        for (var d = 1; d < arguments.length; d += 1) c = (0, arguments[d])(c);
        b.data !== c && (b.data = c);
      }
    },
    tmb = function (a) {
      smb.push(a);
      if (a.attributes) {
        var b = a.attributes,
          c;
        for (c in b) E5.attributes[c] = b[c](E5.attributes[c]);
      }
      a.QZ && $lb(a.QZ);
    },
    F5 = function (a) {
      function b() {
        return void umb(c);
      }
      var c = vmb(!0);
      wmb(c, function () {
        return void a(b, c);
      });
    },
    xmb = function (a) {
      if (G5) {
        var b;
        (b = G5).JC || (b.JC = []);
        G5.JC.push(a);
      }
    },
    wmb = function (a, b) {
      var c = G5;
      G5 = a;
      try {
        return b();
      } finally {
        G5 = c;
      }
    },
    vmb = function (a) {
      var b = {};
      if ((void 0 === a || a) && G5) {
        var c;
        (c = G5).TE || (c.TE = []);
        G5.TE.push(b);
      }
      return b;
    },
    ymb = function (a) {
      var b;
      null == (b = a.TE) || b.forEach(umb);
      a.TE && (a.TE = []);
      if (a.JC) {
        b = g.v(a.JC);
        for (var c = b.next(); !c.done; c = b.next()) (c = c.value), c();
        a.JC = [];
      }
    },
    umb = function (a) {
      if (!a.isDisposed) {
        a.isDisposed = !0;
        var b;
        null == (b = a.W8) || b.call(a);
        ymb(a);
      }
    },
    zmb = function (a, b) {
      if (a.Im && a.Wr) {
        var c = a.Im[b],
          d = a.Wr[b];
        if (c.pJ && c.tE) {
          var e = c.pJ.length - 1;
          H5(c.pJ, d, e);
          H5(c.tE, d, e);
          c.pJ.pop();
          c.tE.pop();
          e = c.pJ[d];
          null != e && e.Wr && (e.Wr[c.tE[d]] = d);
        }
      }
      a.Im &&
        a.Wr &&
        (H5(a.Im, b, a.Im.length - 1),
        H5(a.Wr, b, a.Wr.length - 1),
        a.Im.pop(),
        a.Wr.pop(),
        (c = a.Im[b]),
        (a = a.Wr[b]),
        null == c ? 0 : c.tE) &&
        (c.tE[a] = b);
    },
    H5 = function (a, b, c) {
      -1 === c && (c = a.length - 1);
      var d = a[b];
      a[b] = a[c];
      a[c] = d;
    },
    Amb = function (a) {
      if (0 !== a.state && !a.isDisposed) {
        if (1 === a.state && a.Im)
          for (var b = g.v(a.Im), c = b.next(); !c.done; c = b.next())
            (c = c.value), null != c.AF && Amb(c);
        2 === a.state && null != a.AF && a.AF(a);
        a.state = 0;
      }
    },
    Bmb = function (a, b) {
      wmb(a, function () {
        return b(void 0);
      });
    },
    Cmb = function (a) {
      var b = a.gD;
      a.Daa = 0;
      ymb(a);
      Bmb(a, b);
      if (a.Im && a.Wr) for (b = a.Daa; b < a.Im.length; b++) zmb(a, b);
    },
    Emb = function (a) {
      var b = vmb();
      b.value = null;
      b.W8 = Dmb.bind(null, b);
      b.gD = a;
      b.AF = Cmb;
      b.nib = !0;
      b.state = 2;
      return b;
    },
    Dmb = function (a) {
      if (a.Im) for (var b = 0; b < a.Im.length; b++) zmb(a, b);
    },
    Fmb = function (a, b) {
      return (null === a || "object" !== typeof a) && Object.is(a, b);
    },
    Hmb = function (a) {
      var b = Gmb;
      Gmb = a;
      return b;
    },
    Jmb = function (a, b) {
      null != a.Sp || (a.Sp = []);
      null != a.gA || (a.gA = []);
      Imb(a);
      if (1 === a.Sp.length)
        for (var c = 0; c < a.Mr.length; c++) Jmb(a.Mr[c], a.Vw[c]);
      c = a.Sp.length - 1;
      a.Sp[b] = a.Sp[c];
      a.gA[b] = a.gA[c];
      a.Sp.length--;
      a.gA.length--;
      b < a.Sp.length && ((c = a.gA[b]), (a = a.Sp[b]), Imb(a), (a.Vw[c] = b));
    },
    Imb = function (a) {
      null != a.Mr || (a.Mr = []);
      null != a.Vw || (a.Vw = []);
      null != a.bK || (a.bK = []);
    },
    Lmb = function () {
      var a = [].concat(g.qa(Kmb));
      Kmb.length = 0;
      a = g.v(a);
      for (var b = a.next(); !b.done; b = a.next()) Amb(b.value);
    },
    Mmb = function (a) {
      1 === Kmb.push(a) && Promise.resolve().then(Lmb);
    },
    Nmb = function (a) {
      a = Emb(a);
      a.schedule = Mmb;
      Mmb(a);
    },
    Omb = function (a) {
      a = Emb(a);
      Amb(a);
    },
    I5 = function (a) {
      this.props = a;
      this.B = !1;
    },
    Pmb = function (a) {
      var b = J5;
      try {
        return (J5 = !1), a();
      } finally {
        J5 = b;
      }
    },
    Qmb = function (a) {
      var b = J5;
      try {
        return (J5 = !0), a();
      } finally {
        J5 = b;
      }
    },
    Rmb = function () {
      if (J5)
        throw Error(
          "Reactive components are not allowed to use useState or other memoization based hooks."
        );
      return K5;
    },
    L5 = function (a) {
      I5.call(this, a);
      var b = this;
      this.Kz = [];
      this.D = 0;
      F5(function (c, d) {
        b.G = c;
        b.K = d;
      });
      xmb(function () {
        Smb(b, b.el);
      });
    },
    Smb = function (a, b) {
      if (!a.B && b) {
        a.B = !0;
        q5.tH(a);
        try {
          a.tH();
        } catch (e) {
          var c, d;
          null == (d = E5.JD) ||
            d.call(E5, null == (c = a.j) ? void 0 : c.zy, e);
        }
        a.el = null;
        b.__instance && delete b.__instance;
      }
    },
    Tmb = function (a) {
      if ("function" === typeof a.children) return a.children(), null;
      a = g.v(a.children);
      for (var b = a.next(); !b.done; b = a.next()) (b = b.value), b();
      return null;
    },
    Wmb = function (a, b, c) {
      if (
        Object.hasOwnProperty.call(Umb, a) &&
        ((a = Umb[a]),
        Object.hasOwnProperty.call(a, b) && ((a = a[b]), a instanceof Array))
      ) {
        for (var d = null, e = !1, f = 0, h = a.length; f < h; ++f) {
          var l = a[f],
            m = l.di;
          if (!m) return l.Dd;
          null === d && (d = {});
          m = Object.hasOwnProperty.call(d, m) ? d[m] : (d[m] = c(m));
          if (m === l.Fi) return l.Dd;
          null == m && (e = !0);
        }
        if (e) return null;
      }
      b = Vmb[b];
      return "number" === typeof b ? b : null;
    },
    M5 = function () {
      this.CX = "";
    },
    Ymb = function (a, b, c) {
      if (null === c || void 0 === c) return c;
      var d = Wmb(a.tagName.toLowerCase(), b, function () {
        throw Error("Contingent attribute/property lookups are not supported.");
      });
      if (null === d) return c;
      d = Xmb[d];
      var e;
      if (null == (e = d.Mt) ? 0 : e.call(d, c)) {
        if (d.qs) return d.qs(c);
        throw Error("Value Handler's unwrap function does not exist.");
      }
      c = c && c.Hn ? c.Vk() : c;
      return d.Kq ? d.Kq(a.tagName, b, String(c)) : c;
    },
    Zmb = function (a, b) {
      var c = N5;
      null == a.Fs &&
        null != a.GG &&
        F5(function (d) {
          a.Fs = d;
          Omb(function () {
            for (var e = a.GG, f = [], h = 0; h < e.length; h++) {
              var l = e[h],
                m = b[l]();
              f[h] !== m && ((f[h] = m), c(a, l, m));
            }
          });
        });
    },
    $mb = function (a) {
      var b = document.createTextNode("");
      F5(function (c) {
        b.TU = !0;
        b.Fs = c;
        Omb(function () {
          var d = a();
          null == d && (d = "");
          var e = typeof d;
          if ("object" === e || "function" === e)
            throw Error(
              "Invalid text node kind: " +
                e +
                ". Text nodes must be primitives like numbers, strings, or null, but an object type was passed"
            );
          b.textContent = String(d);
        });
      });
      return b;
    },
    N5 = function (a, b, c) {
      !1 === c && anb.has(b)
        ? cmb(a, b, null)
        : "idomKey" !== b &&
          "skip" !== b &&
          "skipchildren" !== b &&
          "children" !== b &&
          "el" !== b &&
          (b.startsWith("on")
            ? ":" === b[2]
              ? (a["on" + b.slice(3)] = c.f6(0))
              : bnb(a, b, c)
            : "function" !== typeof c || null != a.Fs
            ? (E5.B5 && (c = Ymb(a, b, c)),
              "style" === b
                ? cnb(a, b, c)
                : b.startsWith("prop:")
                ? (a[b.slice(5)] = c)
                : bnb(a, b, c))
            : (a.GG || (a.GG = []), a.GG.push(b)));
    },
    dnb = function (a, b) {
      if (null == b ? 0 : b.el) {
        var c = b.el;
        if ("function" === typeof c) c(a);
        else {
          var d;
          null == (d = c.Z9) || d.call(c, a);
          a._disposeRef ||
            ((a._disposeRef = function () {
              var e;
              null == (e = c.S$) || e.call(c);
              delete a._disposeRef;
            }),
            xmb(function () {
              var e;
              return void (null == (e = a._disposeRef) ? void 0 : e.call(a));
            }));
        }
      }
    },
    gnb = function (a, b, c) {
      ((void 0 === c ? 0 : c) ? enb : fnb)(a, function () {
        O5(b);
      });
    },
    O5 = function (a) {
      if (void 0 !== a && null !== a)
        if (Array.isArray(a)) {
          a = g.v(a);
          for (var b = a.next(); !b.done; b = a.next()) O5(b.value);
        } else if (a instanceof g.Wd || a instanceof g.Zd || a instanceof g.ce)
          rmb(a.toString());
        else if (a instanceof Node) {
          if (u5() !== a)
            throw Error(
              "Encountered a real dom node where a vdom node was expected. Real dom nodes should only come from the reactive renderer, and they can't be passed in JSX expressions directly. Tag name: " +
                a.tagName
            );
          x5 = u5();
        } else if (
          ((b = typeof a), "boolean" === b || "number" === b || "string" === b)
        )
          rmb(a);
        else if ("function" === typeof a) {
          b = t5;
          var c = u5();
          (c && c.TU) || b.insertBefore($mb(a), c);
          x5 = u5();
        } else {
          if ("string" === typeof a.type) {
            a.Cq || nmb(a.type, a.props.idomKey);
            b = t5;
            for (var d in a.props)
              if (a.props[d] !== hnb) {
                var e = a.props[d],
                  f = B5;
                f.push(d);
                f.push(e);
              }
            d = E5.attributes;
            d = void 0 === d ? inb : d;
            e = t5;
            var h = s5(e);
            f = d;
            d = B5;
            h = h.j || (h.j = bmb(d.length));
            for (var l = !h.length || !1, m = 0; m < d.length; m += 2) {
              var n = d[m];
              if (l) h[m] = n;
              else if (h[m] !== n) break;
              var p = d[m + 1];
              if (l || h[m + 1] !== p) (h[m + 1] = p), qmb(e, n, p, f);
            }
            if (m < d.length || m < h.length) {
              for (m = l = m; m < h.length; m += 2) P5[h[m]] = h[m + 1];
              for (m = l; m < d.length; m += 2)
                (l = d[m]),
                  (n = d[m + 1]),
                  P5[l] !== n && qmb(e, l, n, f),
                  (h[m] = l),
                  (h[m + 1] = n),
                  delete P5[l];
              r5(h, d.length);
              for (c in P5) qmb(e, c, void 0, f), delete P5[c];
            }
            c = jnb;
            jnb = e = D5.length;
            for (f = c; f < e; f += 5)
              (0, D5[f])(D5[f + 1], D5[f + 2], D5[f + 3], D5[f + 4]);
            jnb = c;
            r5(D5, c);
            r5(d, 0);
            Zmb(b, a.props);
            (a.props.skip || a.props.skipchildren) && t5.hasChildNodes()
              ? (x5 = t5.lastChild)
              : O5(a.children);
            w5(null);
            x5 = t5;
            t5 = t5.parentNode;
            a.Cq && (a.Cq = !1);
            dnb(b, a.props);
            return b;
          }
          a.type === Zlb ? O5(a.children) : E5.zO(a) || E5.RZ(a);
        }
    },
    knb = function (a) {
      var b;
      return void 0 === (null == (b = a.prototype) ? void 0 : b.bx);
    },
    lnb = function (a) {
      for (var b = 0; b < a.length; b++) {
        var c = void 0,
          d = void 0,
          e = void 0,
          f = a[b];
        f &&
          (null == (e = f._disposeRef) || e.call(f),
          null == (d = f.Fs) || d.call(f),
          f.__instance &&
            ((d = f.__instance), Smb(d, f), null == (c = d.G) || c.call(d)),
          lnb(f.childNodes));
      }
    },
    nnb = function (a) {
      1 === mnb.push(a) &&
        requestAnimationFrame(function () {
          setTimeout(function () {
            var b = [].concat(g.qa(mnb));
            mnb = [];
            b = g.v(b);
            for (var c = b.next(); !c.done; c = b.next()) {
              c = c.value;
              try {
                lnb(c);
              } catch (e) {
                var d = (c = void 0);
                null == (d = (c = E5).JD) || d.call(c, "unknown", e);
              }
            }
          });
        });
    },
    R5 = function (a, b) {
      return Q5.apply(null, [a, b].concat(g.qa(g.Ka.apply(2, arguments))));
    },
    S5 = function (a, b) {
      var c = Q5;
      Q5 = a;
      try {
        return b();
      } finally {
        Q5 = c;
      }
    },
    onb = function (a) {
      Pmb(function () {
        return S5(Ylb, a);
      });
    },
    pnb = function (a) {
      a = document.createTextNode(String(a));
      a.TU = !0;
      return a;
    },
    T5 = function (a) {
      a = typeof a;
      return "string" === a || "number" === a || "boolean" === a;
    },
    qnb = function (a, b) {
      a.parentElement && a.parentElement.replaceChild(b, a);
      return b;
    },
    rnb = function (a, b) {
      var c = a[0].parentElement;
      if (c)
        if (a[0].previousSibling || a[a.length - 1].nextSibling) {
          c.insertBefore(b, a[0]);
          for (var d = a.length - 1; 0 <= d; d--) c.removeChild(a[d]);
        } else (c.textContent = ""), c.appendChild(b);
      return b;
    },
    snb = function (a, b) {
      if (a[0].parentElement)
        for (
          var c = a[0].parentElement,
            d = b.length,
            e = a.length,
            f = d,
            h = 0,
            l = 0,
            m = a[e - 1].nextSibling,
            n = null;
          h < e || l < f;

        )
          if (a[h] === b[l]) h++, l++;
          else {
            for (; a[e - 1] === b[f - 1]; ) e--, f--;
            if (e === h)
              for (
                var p = f < d ? (l ? b[l - 1].nextSibling : b[f - l]) : m;
                l < f;

              )
                c.insertBefore(b[l++], p);
            else if (f === l)
              for (; h < e; ) (n && n.has(a[h])) || c.removeChild(a[h]), h++;
            else if (a[h] === b[f - 1] && b[l] === a[e - 1])
              (p = a[--e].nextSibling),
                c.insertBefore(b[l++], a[h++].nextSibling),
                c.insertBefore(b[--f], p),
                (a[e] = b[f]);
            else {
              if (!n) for (n = new Map(), p = l; p < f; ) n.set(b[p], p++);
              p = n.get(a[h]);
              if (null == p) c.removeChild(a[h]), h++;
              else if (l < p && p < f) {
                for (
                  var q = h, r = 1, t = void 0;
                  ++q < e && q < f && null != (t = n.get(a[q])) && t === p + r;

                )
                  r++;
                if (r > p - l)
                  for (q = a[h]; l < p; ) c.insertBefore(b[l++], q);
                else c.replaceChild(b[l++], a[h++]);
              } else h++;
            }
          }
      return b;
    },
    tnb = function (a) {
      var b;
      Omb(function () {
        var c = S5(U5, a),
          d;
        for (d = b; "function" === typeof c; ) c = c();
        if (null == c ? 0 : c.type)
          throw Error("Encountered a VNode when only real nodes are expected.");
        null == d
          ? (d =
              null == c
                ? document.createTextNode("")
                : T5(c)
                ? pnb(c)
                : c instanceof Node
                ? c
                : 0 === c.length
                ? document.createTextNode("")
                : V5(c))
          : d instanceof Node
          ? null == c
            ? (d = qnb(d, document.createTextNode("")))
            : T5(c)
            ? d.textContent !== String(c) && (d.textContent = String(c))
            : (d =
                c instanceof Node
                  ? qnb(d, c)
                  : 0 === c.length
                  ? qnb(d, document.createTextNode(""))
                  : snb([d], V5(c)))
          : (d =
              null == c
                ? rnb(d, document.createTextNode(""))
                : T5(c)
                ? rnb(d, pnb(c))
                : c instanceof Node
                ? snb(d, [c])
                : 0 === c.length
                ? rnb(d, document.createTextNode(""))
                : snb(d, V5(c)));
        if (b) {
          c = Array.isArray(b) ? b[0] : b;
          var e = c.Iy,
            f = c.Fs;
          c.Fs = void 0;
          c = Array.isArray(d) ? d[0] : d;
          c.Iy = e;
          c.Iy && (c.Iy.BY = c === d ? 1 : d.length);
          c.Fs = f;
        }
        b = d;
      });
      return b;
    },
    V5 = function (a, b) {
      b = void 0 === b ? [] : b;
      if (null == a) return b;
      T5(a) && (a = pnb(a));
      if (a instanceof Node) return unb(b, a), b;
      if (Array.isArray(a)) {
        a = g.v(a);
        for (var c = a.next(); !c.done; c = a.next()) V5(c.value, b);
        return b;
      }
      if ("function" === typeof a) return V5(tnb(a), b);
      if (null == a ? 0 : a.type) {
        var d = a;
        a = S5(U5, function () {
          return R5.apply(null, [d.type, d.props].concat(g.qa(d.children)));
        });
        return V5(a, b);
      }
      throw Error("Unrecognized JSXResult type in flattening.");
    },
    unb = function (a, b) {
      Array.isArray(a) ? a.push(b) : a.appendChild(b);
    },
    wnb = function (a) {
      var b = a.type;
      if (!b.w7) return !1;
      a.props.children = 1 < a.children.length ? a.children : a.children[0];
      var c,
        d = null != (c = a.props.idomKey) ? c : a.type,
        e;
      if (u5() && (null == (e = u5().Iy) ? void 0 : e.key) === d) {
        c = u5().Iy;
        if (!c) throw Error("Reactive data has been lost.");
        if (!b.t0) {
          for (a = 0; a < c.BY; a++) x5 = u5();
          return !0;
        }
        b = b.t0(a.props, c.props);
        a = a.props;
        if ((c = c.props))
          (c.children = a.children), (c.A5 = a.A5), (c.fallback = a.fallback);
        O5(b);
        return !0;
      }
      a = vnb(a.type, a.props);
      a = Array.isArray(a) ? a : [a];
      a = g.v(a);
      for (b = a.next(); !b.done; b = a.next())
        t5.insertBefore(b.value, u5()), (x5 = u5());
      return !0;
    },
    vnb = function (a, b) {
      var c;
      F5(function (d) {
        c = S5(U5, function () {
          return Qmb(function () {
            return a(b);
          });
        });
        var e = Array.isArray(c) ? c[0] : c,
          f,
          h = null != (f = null == b ? void 0 : b.idomKey) ? f : a;
        f = c !== e;
        var l = a.t0;
        e.Fs = d;
        e.Iy = { key: h, props: l ? b : void 0, BY: f ? c.length : 1 };
      });
      return c;
    },
    ynb = function (a, b) {
      var c = g.Ka.apply(2, arguments);
      if (a === Zlb) return c;
      if ("function" === typeof a) {
        var d = b;
        0 < c.length &&
          (null != d || (d = {}),
          1 === c.length ? (d.children = c[0]) : (d.children = c));
        c = xnb(a, d);
        !1 === c && (c = vnb(a, d));
        return c;
      }
      d = document.createElement(a);
      for (var e in b) N5(d, e, b[e]);
      Zmb(d, b);
      V5(c, d);
      dnb(d, b);
      return d;
    },
    xnb = function (a, b) {
      if (a.eib || a.w7) return !1;
      var c = new L5(null != b ? b : {});
      c.C = a;
      var d = Pmb(function () {
        return c.bx(b);
      });
      if (!(d instanceof HTMLElement)) return d;
      d.__instance = c;
      c.el = d;
      c.j = a;
      a.zy = d.tagName.toLowerCase();
      q5.RC(c);
      return d;
    },
    znb = function (a) {
      Nmb(function () {
        var b = a();
        b && xmb(b);
      });
    },
    Anb = function (a) {
      var b = Rmb();
      if (null == b)
        throw Error(
          "A valid hook context was not found. Please ensure you are using components from TSX and not invoking the component function  directly"
        );
      var c = b.D++;
      b.Kz || (b.Kz = []);
      var d = b.Kz;
      d[c] || (d[c] = { key: a, host: b });
      if (a !== d[c].key)
        throw Error(
          "Hook called out of order. Hooks must be invoked unconditionally and in the same order every render. This could happen if you conditionally invoke a hook."
        );
      return d[c];
    },
    Bnb = function (a, b) {
      return (
        !a ||
        a.length !== (null == b ? void 0 : b.length) ||
        a.some(function (c, d) {
          return c !== b[d];
        })
      );
    },
    W5 = function (a, b) {
      Cnb(a, b);
    },
    Dnb = function (a) {
      J5
        ? znb(a)
        : Cnb(function () {
            return Pmb(a);
          }, []);
    },
    Cnb = function (a, b) {
      var c = Anb("onChange"),
        d = Rmb();
      Bnb(c.Ig, b) &&
        ((c.Ig = b), (c.vZ = a), d.Hy || (d.Hy = []), d.Hy.push(c));
    },
    Fnb = function (a) {
      var b = [].concat(g.qa(a));
      a.length = 0;
      a = g.v(b);
      for (b = a.next(); !b.done; b = a.next()) {
        b = b.value;
        Enb(b);
        var c = b.vZ;
        b.vZ = null;
        if ((c = null == c ? void 0 : c())) b.uZ = c;
      }
    },
    Enb = function (a) {
      var b = a.uZ;
      a.uZ = null;
      null == b || b();
    },
    X5 = function (a) {
      var b = [];
      if (J5) return a();
      var c = Anb("useMemoInternal");
      Bnb(c.Ig, b) && ((c.Ig = b), (c.value = a()));
      return c.value;
    },
    Gnb = function (a) {
      var b = null,
        c;
      return {
        value: null,
        Z9: function (d) {
          if (c && d !== c) {
            var e;
            null == (e = b) || e();
            c._disposeRef = void 0;
          }
          c = d;
          b = a(d) || null;
        },
        S$: function () {
          var d;
          null == (d = b) || d();
        },
      };
    },
    Y5 = function () {
      var a = X5(function () {
        return Gnb(function (b) {
          a.value = b;
          return function () {
            a.value = null;
          };
        });
      });
      return a;
    },
    Hnb = function (a) {
      return {
        f6: function () {
          return function (b) {
            !0 !== a(b) && b.stopPropagation();
          };
        },
      };
    },
    Inb = function (a) {
      var b = Rmb(),
        c = X5(function () {
          return { value: "function" === typeof a ? a() : a };
        });
      return [
        c.value,
        function (d) {
          if (null !== K5) throw Error("Can't set state during rendering");
          c.value = "function" === typeof d ? d(c.value) : d;
          b.sM = { OP: b.state, WO: !0 };
          onb(function () {
            if (b.el) {
              var e,
                f = {
                  props: b.props,
                  type: b.j,
                  children: null != (e = b.props.children) ? e : [],
                };
              try {
                gnb(b.el, f, !0);
              } catch (m) {
                var h, l;
                null == (l = E5.JD) ||
                  l.call(E5, null == (h = b.j) ? void 0 : h.zy, m);
              }
            }
          });
        },
      ];
    },
    Jnb = function () {
      return X5(function () {
        var a = null;
        return {
          get value() {
            return a;
          },
          set value(b) {
            if (null !== K5) throw Error("Can't update value during rendering");
            a = b;
          },
        };
      });
    },
    Nnb = function (a) {
      var b = Knb;
      Lnb.push(a);
      Mnb ||
        (b(function () {
          for (var c = g.v(Lnb), d = c.next(); !d.done; d = c.next())
            (d = d.value), d();
          Lnb.length = 0;
          Mnb = !1;
        }),
        (Mnb = !0));
    },
    Knb = function (a) {
      Promise.resolve().then(a);
    },
    Onb = function (a) {
      a = g.v(Inb(a));
      var b = a.next().value,
        c = a.next().value;
      return [
        b,
        function (d) {
          Nnb(function () {
            c(d);
          });
        },
      ];
    },
    Pnb = function (a) {
      function b() {
        var l = g.rU(a.J) ? "Hide more Shorts" : "Hide more videos";
        e(l);
      }
      var c = g.v(Inb("Hide more videos")),
        d = c.next().value,
        e = c.next().value;
      W5(
        function () {
          var l = a.J;
          l.addEventListener("videodatachange", b);
          return function () {
            l.removeEventListener("videodatachange", b);
          };
        },
        [a.J]
      );
      var f = X5(function () {
          return new g.gG(g.tG()).element;
        }),
        h = Y5();
      Dnb(function () {
        h.value.appendChild(f);
      });
      return R5(
        "button",
        {
          class: "ytp-button ytp-collapse",
          "aria-label": d,
          "on:click": Hnb(function () {
            a.action && a.action();
          }),
        },
        R5("div", { class: "ytp-collapse-icon", el: h, skip: !0 })
      );
    },
    Qnb = function (a) {
      function b() {
        var h = g.rU(a.J) ? "More Shorts" : "More videos";
        e(h);
      }
      var c = g.v(Onb("More videos")),
        d = c.next().value,
        e = c.next().value,
        f = Y5();
      W5(
        function () {
          a.iK && (a.iK.value = f.value);
        },
        [a.iK]
      );
      W5(
        function () {
          var h = a.J;
          h.addEventListener("videodatachange", b);
          return function () {
            h.removeEventListener("videodatachange", b);
          };
        },
        [a.J]
      );
      return R5(
        "button",
        {
          el: f,
          class: "ytp-button ytp-expand",
          "on:click": Hnb(function () {
            a.action && a.action();
          }),
        },
        d
      );
    },
    Rnb = function (a, b) {
      var c = Jnb();
      W5(
        function () {
          var d = new g.t2(a, b);
          d.B = !0;
          c.value = d;
          return function () {
            var e;
            null == (e = c.value) || e.dispose();
          };
        },
        [a, b]
      );
      return c;
    },
    Snb = function (a) {
      function b(x) {
        a: {
          var B = g.v([1, 16, 32]);
          for (var F = B.next(); !F.done; F = B.next())
            if (g.SG(x, F.value)) {
              B = !0;
              break a;
            }
          B = !1;
        }
        if (!B) {
          var G;
          null != (G = p.value) &&
            G.ND() &&
            h(g.SG(x, 4) && !g.SG(x, 2) && !g.SG(x, 1024));
        }
      }
      function c() {
        b(a.J.Sb());
      }
      function d(x) {
        b(x.state);
      }
      var e = g.v(Onb(!1)),
        f = e.next().value,
        h = e.next().value,
        l = g.v(Onb(!1));
      e = l.next().value;
      var m = l.next().value,
        n = Y5(),
        p = Rnb(a.J, a.ub),
        q = Y5();
      l = Jnb();
      var r = Jnb();
      W5(
        function () {
          var x = a.J,
            B = g.rU(x) ? 157212 : 172777;
          r.value = new g.J();
          x.wb(q.value, r.value, B);
          x.addEventListener("presentingplayerstatechange", d);
          x.addEventListener("videodatachange", c);
          B = "0" === x.U().controlsType;
          g.Mu(x.getRootNode(), "ytp-pause-overlay-controls-hidden", B);
          return function () {
            x.removeEventListener("videodatachange", c);
            x.removeEventListener("presentingplayerstatechange", d);
            var F;
            null == (F = r.value) || F.dispose();
          };
        },
        [a.J]
      );
      W5(
        function () {
          var x;
          null == (x = p.value) || x.Ea(n.value);
        },
        [p]
      );
      var t = a.J;
      if (f)
        if ((g.Mu(t.getRootNode(), "ytp-expand-pause-overlay", !e), e))
          l.value.focus();
        else {
          var u = p.value;
          g.u2(u);
          u.show();
          q.value.focus();
        }
      q.value && t.Wa(q.value, f && !e);
      return R5(
        "ytp-pause-overlay",
        { el: q, class: "ytp-pause-overlay", "aria-hidden": !f },
        R5(Pnb, {
          J: a.J,
          ub: a.ub,
          action: function () {
            m(!0);
          },
        }),
        R5(Qnb, {
          J: a.J,
          ub: a.ub,
          action: function () {
            m(!1);
          },
          iK: l,
        }),
        R5("div", { el: n, skip: !0 })
      );
    },
    Tnb = function (a) {
      g.U.call(this, { I: "div", S: "ytp-related-on-error-overlay" });
      var b = this;
      this.api = a;
      this.K = this.B = 0;
      this.G = new g.PK(this);
      this.j = [];
      this.suggestionData = [];
      this.columns = this.containerWidth = 0;
      this.title = new g.U({
        I: "h2",
        S: "ytp-related-title",
        xa: "{{title}}",
      });
      this.previous = new g.U({
        I: "button",
        Ma: ["ytp-button", "ytp-previous"],
        W: { "aria-label": "Show previous suggested videos" },
        V: [g.pG()],
      });
      this.ma = new g.s2(function (f) {
        b.suggestions.element.scrollLeft = -f;
      });
      this.D = this.C = 0;
      this.N = !0;
      this.next = new g.U({
        I: "button",
        Ma: ["ytp-button", "ytp-next"],
        W: { "aria-label": "Show more suggested videos" },
        V: [g.qG()],
      });
      g.L(this, this.G);
      a = a.U();
      this.Y = a.D;
      g.L(this, this.title);
      this.title.Ea(this.element);
      this.suggestions = new g.U({ I: "div", S: "ytp-suggestions" });
      g.L(this, this.suggestions);
      this.suggestions.Ea(this.element);
      g.L(this, this.previous);
      this.previous.Ea(this.element);
      this.previous.Ta("click", this.Q1, this);
      g.L(this, this.ma);
      for (var c = { Oz: 0 }; 16 > c.Oz; c = { Oz: c.Oz }, c.Oz++) {
        var d = new g.U({
          I: "a",
          S: "ytp-suggestion-link",
          W: { href: "{{link}}", target: a.Y, "aria-label": "{{aria_label}}" },
          V: [
            {
              I: "div",
              S: "ytp-suggestion-image",
              V: [
                {
                  I: "div",
                  W: { "data-is-live": "{{is_live}}" },
                  S: "ytp-suggestion-duration",
                  xa: "{{duration}}",
                },
              ],
            },
            {
              I: "div",
              S: "ytp-suggestion-title",
              W: { title: "{{hover_title}}" },
              xa: "{{title}}",
            },
            { I: "div", S: "ytp-suggestion-author", xa: "{{views_or_author}}" },
          ],
        });
        g.L(this, d);
        d.Ea(this.suggestions.element);
        var e = d.Ga("ytp-suggestion-link");
        g.Ur(e, "transitionDelay", c.Oz / 20 + "s");
        this.G.T(
          e,
          "click",
          (function (f) {
            return function (h) {
              var l = f.Oz,
                m = b.suggestionData[l],
                n = m.sessionData;
              g.cS(b.api.U()) &&
              b.api.L(
                "web_player_log_click_before_generating_ve_conversion_params"
              )
                ? (b.api.rb(b.j[l].element),
                  (l = m.Xk()),
                  (m = {}),
                  g.rSa(b.api, m, "emb_rel_err"),
                  (l = g.Ml(l, m)),
                  g.LU(l, b.api, h))
                : g.KU(h, b.api, b.Y, n || void 0) &&
                  b.api.Ko(m.videoId, n, m.playlistId);
            };
          })(c)
        );
        this.j.push(d);
      }
      g.L(this, this.next);
      this.next.Ea(this.element);
      this.next.Ta("click", this.P1, this);
      this.G.T(this.api, "videodatachange", this.onVideoDataChange);
      this.resize(this.api.ob().getPlayerSize());
      this.onVideoDataChange();
      this.show();
    },
    Unb = function (a, b) {
      if (
        a.api
          .U()
          .L("web_player_log_click_before_generating_ve_conversion_params")
      )
        for (
          var c = Math.floor(-a.C / (a.D + a.B)),
            d = Math.min(c + a.columns, a.suggestionData.length) - 1;
          c <= d;
          c++
        )
          a.api.Wa(a.j[c].element, b);
    },
    Vnb = function (a) {
      a.next.element.style.bottom = a.K + "px";
      a.previous.element.style.bottom = a.K + "px";
      var b = a.C,
        c = a.containerWidth - a.suggestionData.length * (a.D + a.B);
      g.Mu(a.element, "ytp-scroll-min", 0 <= b);
      g.Mu(a.element, "ytp-scroll-max", b <= c);
    },
    Wnb = function (a) {
      for (var b = 0; b < a.suggestionData.length; b++) {
        var c = a.suggestionData[b],
          d = a.j[b],
          e = c.shortViewCount ? c.shortViewCount : c.author,
          f = c.Xk(),
          h = a.api.U();
        if (
          g.cS(h) &&
          !h.L("web_player_log_click_before_generating_ve_conversion_params")
        ) {
          var l = {};
          g.dU(a.api, "addEmbedsConversionTrackingParams", [l]);
          f = g.Ml(f, g.CP(l, "emb_rel_err"));
        }
        d.element.style.display = "";
        l = d.Ga("ytp-suggestion-title");
        g.Yu.test(c.title)
          ? (l.dir = "rtl")
          : g.zeb.test(c.title) && (l.dir = "ltr");
        l = d.Ga("ytp-suggestion-author");
        g.Yu.test(e) ? (l.dir = "rtl") : g.zeb.test(e) && (l.dir = "ltr");
        d.update({
          views_or_author: e,
          duration: c.isLivePlayback
            ? "Live"
            : c.lengthSeconds
            ? g.aH(c.lengthSeconds)
            : "",
          link: f,
          hover_title: c.title,
          title: c.title,
          aria_label: c.ariaLabel || null,
          is_live: c.isLivePlayback,
        });
        e = d.Ga("ytp-suggestion-image");
        f = c.Vg();
        e.style.backgroundImage = f ? "url(" + f + ")" : "";
        h.L("web_player_log_click_before_generating_ve_conversion_params") &&
          (a.api.ag(d.element, d),
          (c = (c = c.sessionData) && c.itct) && a.api.hg(d.element, c));
      }
      for (; b < a.j.length; b++) a.j[b].element.style.display = "none";
      Vnb(a);
    },
    Z5 = function (a) {
      g.VV.call(this, a);
      var b = this;
      this.j = null;
      var c = a.U(),
        d = {
          I: "svg",
          W: {
            fill: "#fff",
            height: "100%",
            viewBox: "0 0 24 24",
            width: "100%",
          },
          V: [
            { I: "path", W: { d: "M0 0h24v24H0V0z", fill: "none" } },
            {
              I: "path",
              W: {
                d: "M21.58 7.19c-.23-.86-.91-1.54-1.77-1.77C18.25 5 12 5 12 5s-6.25 0-7.81.42c-.86.23-1.54.91-1.77 1.77C2 8.75 2 12 2 12s0 3.25.42 4.81c.23.86.91 1.54 1.77 1.77C5.75 19 12 19 12 19s6.25 0 7.81-.42c.86-.23 1.54-.91 1.77-1.77C22 15.25 22 12 22 12s0-3.25-.42-4.81zM10 15V9l5.2 3-5.2 3z",
              },
            },
          ],
        };
      g.kS(c) &&
        (d = {
          I: "svg",
          W: {
            fill: "none",
            height: "100%",
            viewBox: "0 0 25 25",
            width: "100%",
          },
          V: [
            {
              I: "g",
              W: { "clip-path": "url(#clip0)", fill: "#fff" },
              V: [
                {
                  I: "path",
                  W: {
                    d: "M12.57.98C6.21.98 1.05 6.14 1.05 12.5c0 6.36 5.16 11.52 11.52 11.52 6.36 0 11.52-5.16 11.52-11.52C24.09 6.14 18.93.98 12.57.98zm0 18.34c-3.77 0-6.82-3.05-6.82-6.82 0-3.77 3.05-6.82 6.82-6.82 3.77 0 6.82 3.05 6.82 6.82 0 3.77-3.05 6.82-6.82 6.82z",
                  },
                },
                {
                  I: "path",
                  W: {
                    d: "M12.57 6.52c-3.29 0-5.98 2.68-5.98 5.98 0 3.3 2.68 5.98 5.98 5.98 3.3 0 5.98-2.68 5.98-5.98 0-3.3-2.69-5.98-5.98-5.98zm-2.25 9.38V9.1l5.88 3.4-5.88 3.4z",
                  },
                },
              ],
            },
            {
              I: "defs",
              V: [
                {
                  I: "clipPath",
                  W: { id: "clip0" },
                  V: [
                    {
                      I: "path",
                      W: {
                        d: "M0 0h24v24H0z",
                        fill: "#fff",
                        transform: "translate(.57 .5)",
                      },
                    },
                  ],
                },
              ],
            },
          ],
        });
      var e = { target: c.Y },
        f = ["ytp-small-redirect"];
      c.C
        ? f.push("no-link")
        : ((c = g.AS(c)),
          (e.href = c),
          (e["aria-label"] = "Visit YouTube to search for more videos"));
      var h = new g.U({ I: "a", Ma: f, W: e, V: [d] });
      h.Ea(this.element);
      a.wb(h.element, this, 178053);
      this.T(h.element, "click", function (l) {
        Xnb(b, l, h.element);
      });
      g.L(this, h);
      a.U().C ||
        ((this.j = new Tnb(a)), this.j.Ea(this.element), g.L(this, this.j));
      this.T(a, "videodatachange", function () {
        b.show();
      });
      this.resize(this.api.ob().getPlayerSize());
    },
    Xnb = function (a, b, c) {
      b.preventDefault();
      a.api.rb(c);
      b = c.getAttribute("href");
      c = {};
      g.dU(a.api, "addEmbedsConversionTrackingParams", [c]);
      b = g.kd(c) ? b : g.Ml(b, c);
      g.Nn(b);
    },
    Ynb = function (a, b) {
      a.Ga("ytp-error-content").style.paddingTop = "0px";
      var c = a.Ga("ytp-error-content"),
        d = c.clientHeight;
      a.j && a.j.resize(b, b.height - d);
      c.style.paddingTop =
        (b.height - (a.j ? a.j.element.clientHeight : 0)) / 2 - d / 2 + "px";
    },
    aob = function (a, b) {
      var c = a.api.U(),
        d;
      b.reason &&
        (Znb(b.reason) ? (d = g.oG(b.reason)) : (d = g.WV(g.nG(b.reason))),
        a.le(d, "content"));
      var e;
      b.subreason &&
        (Znb(b.subreason)
          ? (e = g.oG(b.subreason))
          : (e = g.WV(g.nG(b.subreason))),
        a.le(e, "subreason"));
      if (b.proceedButton && b.proceedButton.buttonRenderer) {
        d = a.Ga("ytp-error-content-wrap-subreason");
        b = b.proceedButton.buttonRenderer;
        var f = g.nf("A");
        if (
          b.text &&
          b.text.simpleText &&
          ((e = b.text.simpleText),
          (f.textContent = e),
          !$nb(d, e) && (!c.C || c.embedsErrorLinks))
        ) {
          var h;
          c =
            null == (h = g.T(null == b ? void 0 : b.navigationEndpoint, g.zG))
              ? void 0
              : h.url;
          var l;
          h =
            null == (l = g.T(null == b ? void 0 : b.navigationEndpoint, g.zG))
              ? void 0
              : l.target;
          c &&
            (f.setAttribute("href", c),
            a.api.wb(f, a, 178424),
            a.T(f, "click", function (m) {
              Xnb(a, m, f);
            }));
          h && f.setAttribute("target", h);
          l = g.nf("DIV");
          l.appendChild(f);
          d.appendChild(l);
        }
      }
    },
    Znb = function (a) {
      if (a.runs)
        for (var b = 0; b < a.runs.length; b++)
          if (a.runs[b].navigationEndpoint) return !0;
      return !1;
    },
    $nb = function (a, b) {
      a = g.Xe("A", a);
      for (var c = 0; c < a.length; c++) if (a[c].textContent === b) return !0;
      return !1;
    },
    $5 = function (a, b) {
      g.U.call(this, {
        I: "a",
        Ma: ["ytp-impression-link"],
        W: {
          target: "{{target}}",
          href: "{{url}}",
          "aria-label": "Watch on YouTube",
        },
        V: [
          {
            I: "div",
            S: "ytp-impression-link-content",
            W: { "aria-hidden": "true" },
            V: [
              { I: "div", S: "ytp-impression-link-text", xa: "Watch on" },
              { I: "div", S: "ytp-impression-link-logo", V: [Xlb()] },
            ],
          },
        ],
      });
      this.api = a;
      this.ub = b;
      this.updateValue("target", a.U().Y);
      this.api.wb(this.element, this, 96714);
      this.T(this.api, "presentingplayerstatechange", this.aj);
      this.T(this.api, "videoplayerreset", this.j);
      this.T(this.element, "click", this.onClick);
      this.j();
    },
    bob = function (a) {
      var b = {};
      g.dU(a.api, "addEmbedsConversionTrackingParams", [b]);
      return g.Ml(a.api.getVideoUrl(), g.CP(b, "emb_imp_woyt"));
    },
    a6 = function (a) {
      g.U.call(this, {
        I: "div",
        Ma: ["ytp-mobile-a11y-hidden-seek-button"],
        V: [
          {
            I: "button",
            Ma: ["ytp-mobile-a11y-hidden-seek-button-rewind", "ytp-button"],
            W: { "aria-label": "Rewind 10 seconds", "aria-hidden": "false" },
          },
          {
            I: "button",
            Ma: ["ytp-mobile-a11y-hidden-seek-button-forward", "ytp-button"],
            W: {
              "aria-label": "Fast forward 10 seconds",
              "aria-hidden": "false",
            },
          },
        ],
      });
      this.api = a;
      this.j = this.Ga("ytp-mobile-a11y-hidden-seek-button-rewind");
      this.forwardButton = this.Ga(
        "ytp-mobile-a11y-hidden-seek-button-forward"
      );
      this.api.wb(this.j, this, 141902);
      this.api.wb(this.forwardButton, this, 141903);
      this.T(this.api, "presentingplayerstatechange", this.aj);
      this.T(this.j, "click", this.B);
      this.T(this.forwardButton, "click", this.C);
      this.aj();
    },
    b6 = function (a) {
      g.U.call(this, {
        I: "div",
        S: "ytp-muted-autoplay-endscreen-overlay",
        V: [
          {
            I: "div",
            S: "ytp-muted-autoplay-end-panel",
            V: [
              {
                I: "button",
                Ma: ["ytp-muted-autoplay-end-text", "ytp-button"],
                xa: "{{text}}",
              },
            ],
          },
        ],
      });
      this.api = a;
      this.D = this.Ga("ytp-muted-autoplay-end-panel");
      this.B = !1;
      this.api.wb(this.element, this, 52428);
      this.T(this.api, "presentingplayerstatechange", this.C);
      this.T(a, "onMutedAutoplayStarts", this.onMutedAutoplayStarts);
      this.Ta("click", this.onClick);
      this.hide();
    },
    c6 = function (a) {
      var b = a.U();
      g.U.call(this, {
        I: "a",
        Ma: ["ytp-watermark", "yt-uix-sessionlink"],
        W: {
          target: b.Y,
          href: "{{url}}",
          "aria-label": g.lK("Watch on $WEBSITE", { WEBSITE: g.pS(b) }),
          "data-sessionlink": "feature=player-watermark",
        },
        V: [Xlb()],
      });
      this.api = a;
      this.j = null;
      this.B = !1;
      this.state = a.Sb();
      a.wb(this.element, this, 76758);
      this.T(a, "videodatachange", this.x0);
      this.T(a, "videodatachange", this.onVideoDataChange);
      this.T(a, "presentingplayerstatechange", this.onStateChange);
      this.T(a, "appresize", this.Jb);
      this.Gc(this.state);
      this.onVideoDataChange();
      this.Jb(a.ob().getPlayerSize());
    },
    eob = function (a) {
      g.U.call(this, {
        I: "div",
        S: "ytp-muted-autoplay-overlay",
        V: [
          {
            I: "div",
            S: "ytp-muted-autoplay-bottom-buttons",
            V: [
              {
                I: "button",
                Ma: ["ytp-muted-autoplay-equalizer", "ytp-button"],
                W: { "aria-label": "Muted playback indicator" },
                V: [
                  {
                    I: "div",
                    Ma: ["ytp-muted-autoplay-equalizer-icon"],
                    V: [
                      {
                        I: "svg",
                        W: {
                          height: "100%",
                          version: "1.1",
                          viewBox: "-4 -4 24 24",
                          width: "100%",
                        },
                        V: [
                          {
                            I: "g",
                            W: { fill: "#fff" },
                            V: [
                              {
                                I: "rect",
                                S: "ytp-equalizer-bar-left",
                                W: { height: "9", width: "4", x: "1", y: "7" },
                              },
                              {
                                I: "rect",
                                S: "ytp-equalizer-bar-middle",
                                W: { height: "14", width: "4", x: "6", y: "2" },
                              },
                              {
                                I: "rect",
                                S: "ytp-equalizer-bar-right",
                                W: {
                                  height: "12",
                                  width: "4",
                                  x: "11",
                                  y: "4",
                                },
                              },
                            ],
                          },
                        ],
                      },
                    ],
                  },
                ],
              },
            ],
          },
        ],
      });
      var b = this;
      this.api = a;
      this.bottomButtons = this.Ga("ytp-muted-autoplay-bottom-buttons");
      this.Ga("ytp-muted-autoplay-equalizer");
      this.C = new g.Au(this.f7, 4e3, this);
      this.B = !1;
      a.wb(this.element, this, 39306);
      this.T(a, "presentingplayerstatechange", this.zL);
      this.T(a, "onMutedAutoplayStarts", function () {
        cob(b);
        b.zL();
        dob(b);
        b.B = !1;
      });
      this.T(a, "onAutoplayBlocked", this.onAutoplayBlocked);
      this.Ta("click", this.onClick);
      this.T(a, "onMutedAutoplayEnds", this.onMutedAutoplayEnds);
      this.hide();
      g.eU(a.app) && (cob(this), this.zL(), dob(this));
    },
    dob = function (a) {
      a.Fb && a.j && (a.j.show(), a.C.start());
    },
    cob = function (a) {
      a.watermark ||
        ((a.watermark = new c6(a.api)),
        g.L(a, a.watermark),
        a.watermark.Ea(a.bottomButtons, 0),
        g.Mu(a.watermark.element, "ytp-muted-autoplay-watermark", !0),
        (a.j = new g.MG(a.watermark, 0, !0, 100)),
        g.L(a, a.j));
    },
    d6 = function (a, b) {
      g.U.call(this, {
        I: "div",
        S: "ytp-pause-overlay",
        W: { tabIndex: "-1" },
      });
      var c = this;
      this.api = a;
      this.C = new g.PK(this);
      this.D = new g.MG(
        this,
        1e3,
        !1,
        100,
        function () {
          c.j.B = !1;
        },
        function () {
          c.j.B = !0;
        }
      );
      this.B = !1;
      this.expandButton = new g.U({
        I: "button",
        Ma: ["ytp-button", "ytp-expand"],
        xa: g.rU(this.api) ? "More Shorts" : "More videos",
      });
      "0" === a.U().controlsType &&
        g.Iu(a.getRootNode(), "ytp-pause-overlay-controls-hidden");
      g.L(this, this.C);
      g.L(this, this.D);
      var d = new g.U({
        I: "button",
        Ma: ["ytp-button", "ytp-collapse"],
        W: {
          "aria-label": g.rU(this.api)
            ? "Hide more Shorts"
            : "Hide more videos",
        },
        V: [{ I: "div", S: "ytp-collapse-icon", V: [g.tG()] }],
      });
      g.L(this, d);
      d.Ea(this.element);
      d.Ta("click", this.G, this);
      g.L(this, this.expandButton);
      this.expandButton.Ea(this.element);
      this.expandButton.Ta("click", this.K, this);
      this.j = new g.t2(a, b);
      g.L(this, this.j);
      this.j.B = !1;
      this.j.Ea(this.element);
      g.rU(this.api)
        ? this.api.wb(this.element, this, 157212)
        : this.api.wb(this.element, this, 172777);
      this.C.T(this.api, "presentingplayerstatechange", this.Ra);
      this.C.T(this.api, "videodatachange", this.Ra);
      this.hide();
    },
    e6 = function (a) {
      g.U.call(this, {
        I: "div",
        Ma: ["ytp-player-content", "ytp-iv-player-content"],
        V: [
          {
            I: "div",
            S: "ytp-countdown-timer",
            V: [
              {
                I: "svg",
                W: {
                  height: "100%",
                  version: "1.1",
                  viewBox: "0 0 72 72",
                  width: "100%",
                },
                V: [
                  {
                    I: "circle",
                    S: "ytp-svg-countdown-timer-ring",
                    W: {
                      cx: "-36",
                      cy: "36",
                      "fill-opacity": "0",
                      r: "33.5",
                      stroke: "#FFFFFF",
                      "stroke-dasharray": "211",
                      "stroke-dashoffset": "-211",
                      "stroke-width": "4",
                      transform: "rotate(-90)",
                    },
                  },
                  {
                    I: "circle",
                    S: "ytp-svg-countdown-timer-background",
                    W: {
                      cx: "-36",
                      cy: "36",
                      "fill-opacity": "0",
                      r: "33.5",
                      stroke: "#FFFFFF",
                      "stroke-opacity": "0.3",
                      "stroke-width": "4",
                      transform: "rotate(-90)",
                    },
                  },
                ],
              },
              { I: "span", S: "ytp-countdown-timer-time", xa: "{{duration}}" },
            ],
          },
        ],
      });
      this.api = a;
      this.K = this.Ga("ytp-svg-countdown-timer-ring");
      this.j = null;
      this.D = this.C = 0;
      this.B = !1;
      this.G = 0;
      this.api.wb(this.element, this, 159628);
    },
    gob = function (a) {
      a.j ||
        ((a.C = 5e3),
        (a.D = (0, g.FD)()),
        (a.j = new g.yu(function () {
          fob(a);
        }, null)),
        fob(a));
    },
    fob = function (a) {
      if (!a.B) {
        var b = Math.min((0, g.FD)() - a.D, a.C);
        var c = a.C - b;
        b = 0 === a.C ? 0 : Math.max(c / a.C, 0);
        c = Math.round(c / 1e3);
        a.K.setAttribute("stroke-dashoffset", "" + -211 * (b + 1));
        a.updateValue("duration", c);
        0 >= b && a.j ? f6(a) : a.j && a.j.start();
      }
    },
    f6 = function (a) {
      a.j && (a.j.dispose(), (a.j = null), (a.B = !1));
    },
    iob = function (a) {
      g.zV.call(this, a);
      this.J = a;
      this.j = new g.PK(this);
      this.B = null;
      this.N = !1;
      this.countdownTimer = null;
      this.Y = !1;
      hob(this);
      g.L(this, this.j);
      this.load();
    },
    kob = function (a) {
      var b = g.Ocb(a.J);
      b !== a.Y &&
        ((a.Y = b),
        a.G && (a.G.dispose(), (a.G = null)),
        a.C && (a.C.dispose(), (a.C = null)),
        a.D && (a.D.dispose(), (a.D = null)),
        a.B && (a.B.stop(), a.B.dispose(), (a.B = null)),
        b &&
          ((b = g.gU(a.J)),
          g.rU(a.J) &&
            ((a.D = new g.U({
              I: "div",
              S: "ytp-pause-overlay-backdrop",
              W: { tabIndex: "-1" },
            })),
            g.L(a, a.D),
            g.uU(a.J, a.D.element, 4),
            (a.B = new g.MG(a.D, 1e3, !1, 100)),
            g.L(a, a.B),
            a.D.hide()),
          (a.G = new g.U({
            I: "div",
            S: "ytp-pause-overlay-container",
            W: { tabIndex: "-1" },
          })),
          g.L(a, a.G),
          a.J.L("embeds_web_enable_keto_pause_overlay")
            ? gnb(a.G.element, R5(Snb, { J: a.J, ub: b }))
            : ((a.C = new d6(a.J, b)), g.L(a, a.C), a.C.Ea(a.G.element)),
          g.uU(a.J, a.G.element, 4),
          job(a, a.J.Sb())));
    },
    job = function (a, b) {
      a.B &&
        ((!g.SG(b, 4) && !g.SG(b, 2)) || g.SG(b, 1024)
          ? a.B.hide()
          : a.B.show());
    },
    hob = function (a) {
      var b = a.J;
      a = !!g.rU(b);
      g.Mu(b.getRootNode(), "ytp-shorts-mode", a);
      if ((b = b.getVideoData())) b.CU = a;
    },
    g6 = function (a, b) {
      var c = a.J.U();
      a = {
        adSource: "EMBEDS_AD_SOURCE_YOUTUBE",
        breakType:
          0 === a.J.getCurrentTime()
            ? "EMBEDS_AD_BREAK_TYPE_PRE_ROLL"
            : 0 === a.J.getPlayerState()
            ? "EMBEDS_AD_BREAK_TYPE_POST_ROLL"
            : "EMBEDS_AD_BREAK_TYPE_MID_ROLL",
        embedUrl: g.Zea(a.J.U().loaderUrl),
        eventType: b,
        youtubeHost: g.Gl(a.J.U().Ka) || "",
      };
      c.L("embeds_add_player_mode_to_ad_events") &&
        (a.embeddedPlayerMode = c.Da);
      g.qD("embedsAdEvent", a);
    };
  var q5 = {
    Ggb: function () {},
    Jib: function () {},
    Jgb: function () {},
    kV: function () {},
    Hgb: function () {},
    q5: function () {},
    RC: function () {},
    tH: function () {},
    r5: function () {},
  };
  var dmb = Object.prototype.hasOwnProperty;
  amb.prototype = Object.create(null);
  var inb = gmb();
  var C5 = null;
  var omb =
    ("undefined" !== typeof Node && Node.prototype.getRootNode) ||
    function () {
      for (var a = this, b = a; a; ) (b = a), (a = a.parentNode);
      return b;
    };
  var v5 = null,
    x5 = null,
    t5 = null,
    z5 = null,
    A5 = [],
    y5 = lmb,
    B5 = [],
    fnb = (function (a) {
      return pmb(function (b, c, d) {
        t5 = x5 = b;
        x5 = null;
        c(d);
        w5(null);
        x5 = t5;
        t5 = t5.parentNode;
        return b;
      }, a);
    })(),
    enb = (function (a) {
      return pmb(function (b, c, d) {
        var e = { nextSibling: b };
        x5 = e;
        c(d);
        t5 && w5(b.nextSibling);
        return e === x5 ? null : x5;
      }, a);
    })();
  var D5 = [],
    jnb = 0;
  var P5 = new amb();
  var E5 = {
      zH: !1,
      attributes: gmb(),
      JD: function (a, b) {
        throw b;
      },
      RZ: function () {
        throw Error("renderComponent not implemented");
      },
      zO: function () {
        return !1;
      },
      Saa: function (a) {
        if (E5.zH) return a;
      },
      B5: g.adb,
    },
    smb = [];
  var G5 = null;
  var Gmb = null,
    lob = {
      version: 0,
      uib: 0,
      nz: !1,
      Mr: void 0,
      bK: void 0,
      Vw: void 0,
      NP: 0,
      Sp: void 0,
      gA: void 0,
      c5: !1,
      FV: !1,
      xaa: function () {
        return !1;
      },
      yaa: function () {},
      d5: function () {},
      Ugb: function () {},
    };
  var mob = Symbol("UNSET"),
    nob = Symbol("COMPUTING"),
    oob = Symbol("ERRORED");
  Object.assign({}, lob, {
    value: mob,
    nz: !0,
    error: null,
    aW: Fmb,
    xaa: function (a) {
      return a.value === mob || a.value === nob;
    },
    yaa: function (a) {
      if (a.value === nob) throw Error("Detected cycle in computations.");
      var b = a.value;
      a.value = nob;
      a && (a.NP = 0);
      var c = Hmb(a);
      try {
        var d = a.Sgb();
      } catch (h) {
        (d = oob), (a.error = h);
      } finally {
        if (
          (Hmb(c), a && void 0 !== a.Mr && void 0 !== a.Vw && void 0 !== a.bK)
        ) {
          var e, f;
          if (
            a.FV ||
            0 <
              (null !=
              (f = null == a ? void 0 : null == (e = a.Sp) ? void 0 : e.length)
                ? f
                : 0)
          )
            for (c = a.NP; c < a.Mr.length; c++) Jmb(a.Mr[c], a.Vw[c]);
          for (; a.Mr.length > a.NP; ) a.Mr.pop(), a.bK.pop(), a.Vw.pop();
        }
      }
      b !== mob && b !== oob && d !== oob && a.aW(b, d)
        ? (a.value = b)
        : ((a.value = d), a.version++);
    },
  });
  Object.assign({}, lob, { aW: Fmb, value: void 0 });
  Object.assign({}, lob, {
    FV: !0,
    c5: !1,
    d5: function (a) {
      null !== a.schedule && a.schedule(a.iK);
    },
    fib: !1,
    Ogb: function () {},
  });
  var Kmb = [];
  I5.prototype.RC = function () {};
  I5.prototype.q5 = function () {};
  I5.prototype.tH = function () {};
  I5.prototype.r5 = function () {};
  var J5 = !1;
  var K5 = null;
  g.w(L5, I5);
  L5.prototype.bx = function (a) {
    var b = this,
      c = K5;
    K5 = this;
    this.D = 0;
    try {
      return wmb(this.K, function () {
        return b.C(a);
      });
    } finally {
      K5 = c;
    }
  };
  var anb = new Set(
    "allowfullscreen async autofocus autoplay checked controls default defer disabled formnovalidate hidden ismap itemscope jsshadow jsslot loop multiple muted novalidate open playsinline readonly required reversed scoped seamless selected spellcheck sortable typemustmatch".split(
      " "
    )
  );
  var Vmb = {
      align: 1,
      alt: 1,
      "aria-activedescendant": 10,
      "aria-atomic": 1,
      "aria-autocomplete": 1,
      "aria-busy": 1,
      "aria-checked": 1,
      "aria-controls": 10,
      "aria-current": 1,
      "aria-disabled": 1,
      "aria-dropeffect": 1,
      "aria-expanded": 1,
      "aria-haspopup": 1,
      "aria-hidden": 1,
      "aria-invalid": 1,
      "aria-label": 1,
      "aria-labelledby": 10,
      "aria-level": 1,
      "aria-live": 1,
      "aria-multiline": 1,
      "aria-multiselectable": 1,
      "aria-orientation": 1,
      "aria-owns": 10,
      "aria-posinset": 1,
      "aria-pressed": 1,
      "aria-readonly": 1,
      "aria-relevant": 1,
      "aria-required": 1,
      "aria-selected": 1,
      "aria-setsize": 1,
      "aria-sort": 1,
      "aria-valuemax": 1,
      "aria-valuemin": 1,
      "aria-valuenow": 1,
      "aria-valuetext": 1,
      async: 8,
      autocapitalize: 1,
      autocomplete: 1,
      autocorrect: 1,
      autofocus: 1,
      autoplay: 1,
      bgcolor: 1,
      border: 1,
      cellpadding: 1,
      cellspacing: 1,
      checked: 1,
      cite: 3,
      class: 1,
      color: 1,
      cols: 1,
      colspan: 1,
      contenteditable: 1,
      controls: 1,
      datetime: 1,
      dir: 8,
      disabled: 1,
      download: 1,
      draggable: 1,
      enctype: 1,
      face: 1,
      for: 10,
      formenctype: 1,
      frameborder: 1,
      height: 1,
      hidden: 1,
      href: 4,
      hreflang: 1,
      id: 10,
      ismap: 1,
      itemid: 1,
      itemprop: 1,
      itemref: 1,
      itemscope: 1,
      itemtype: 1,
      label: 1,
      lang: 1,
      list: 10,
      loading: 8,
      loop: 1,
      max: 1,
      maxlength: 1,
      media: 1,
      min: 1,
      minlength: 1,
      multiple: 1,
      muted: 1,
      name: 10,
      nonce: 1,
      open: 1,
      placeholder: 1,
      poster: 3,
      preload: 1,
      rel: 1,
      required: 1,
      reversed: 1,
      role: 1,
      rows: 1,
      rowspan: 1,
      selected: 1,
      shape: 1,
      size: 1,
      sizes: 1,
      slot: 1,
      span: 1,
      spellcheck: 1,
      src: 4,
      srcset: 11,
      start: 1,
      step: 1,
      style: 5,
      summary: 1,
      tabindex: 1,
      target: 8,
      title: 1,
      translate: 1,
      type: 1,
      valign: 1,
      value: 1,
      width: 1,
      wrap: 1,
    },
    Umb = {
      a: { href: [{ Dd: 3 }] },
      area: { href: [{ Dd: 3 }] },
      audio: { src: [{ Dd: 3 }] },
      button: { formaction: [{ Dd: 3 }], formmethod: [{ Dd: 1 }] },
      form: { action: [{ Dd: 3 }], method: [{ Dd: 1 }] },
      iframe: { srcdoc: [{ Dd: 2 }] },
      img: { src: [{ Dd: 3 }] },
      input: {
        accept: [{ Dd: 1 }],
        formaction: [{ Dd: 3 }],
        formmethod: [{ Dd: 1 }],
        pattern: [{ Dd: 1 }],
        readonly: [{ Dd: 1 }],
        src: [{ Dd: 3 }],
      },
      link: {
        href: [
          { Dd: 3, di: "rel", Fi: "alternate" },
          { Dd: 3, di: "rel", Fi: "author" },
          { Dd: 3, di: "rel", Fi: "bookmark" },
          { Dd: 3, di: "rel", Fi: "canonical" },
          { Dd: 3, di: "rel", Fi: "cite" },
          { Dd: 3, di: "rel", Fi: "help" },
          { Dd: 3, di: "rel", Fi: "icon" },
          { Dd: 3, di: "rel", Fi: "license" },
          { Dd: 3, di: "rel", Fi: "next" },
          { Dd: 3, di: "rel", Fi: "prefetch" },
          { Dd: 3, di: "rel", Fi: "dns-prefetch" },
          { Dd: 3, di: "rel", Fi: "prerender" },
          { Dd: 3, di: "rel", Fi: "preconnect" },
          { Dd: 3, di: "rel", Fi: "preload" },
          { Dd: 3, di: "rel", Fi: "prev" },
          { Dd: 3, di: "rel", Fi: "search" },
          { Dd: 3, di: "rel", Fi: "subresource" },
        ],
      },
      script: { defer: [{ Dd: 1 }] },
      source: { src: [{ Dd: 3 }] },
      textarea: { readonly: [{ Dd: 1 }] },
      video: { src: [{ Dd: 3 }] },
    };
  g.vv.getInstance();
  M5.prototype.toString = function () {
    return this.CX;
  };
  var pob = /^(?!javascript:)(?:[a-z0-9+.-]+:|[^&:\/?#]*(?:[\/?#]|$))/i,
    h6 = {},
    Xmb =
      ((h6[1] = { Kq: null, Mt: null, qs: null }),
      (h6[2] = {
        Kq: function () {
          return g.ee.toString();
        },
        Mt: function (a) {
          return a instanceof g.fe;
        },
        qs: function (a) {
          return g.je(a);
        },
      }),
      (h6[3] = {
        Kq: function (a, b, c) {
          return pob.test(c) ? c : g.ee.toString();
        },
        Mt: function (a) {
          return a instanceof g.Zd;
        },
        qs: function (a) {
          return g.$d(a);
        },
      }),
      (h6[4] = {
        Kq: function () {
          return g.ee.toString();
        },
        Mt: function (a) {
          return a instanceof g.Wd;
        },
        qs: function (a) {
          return g.Xd(a);
        },
      }),
      (h6[5] = { Kq: null, Mt: null, qs: null }),
      (h6[7] = { Kq: null, Mt: null, qs: null }),
      (h6[8] = { Kq: null, Mt: null, qs: null }),
      (h6[10] = {
        Kq: null,
        Mt: function (a) {
          return a instanceof M5 || a instanceof g.Qd;
        },
        qs: function (a) {
          if (a instanceof g.Qd) a = g.Rd(a);
          else {
            if (!(a instanceof M5 && a.constructor === M5))
              throw Error("Unwrap non-TrustedId");
            a = a.CX;
          }
          return a;
        },
      }),
      h6);
  var qob = gmb(),
    bnb = qob.__default,
    cnb = qob.style;
  var hnb = Symbol("ATTR_TAG_VALUE");
  var rob = {};
  tmb({
    attributes:
      ((rob.__default = function () {
        return N5;
      }),
      (rob.style = function () {
        return N5;
      }),
      rob),
  });
  var mnb = [];
  E5.RZ = function (a) {
    try {
      var b = a.type,
        c = b.zy;
      if (b === Tmb) (a.props.children = a.children), b(a.props);
      else if (knb(b) && !E5.zH) O5(b(a.props));
      else {
        0 < a.children.length && (a.props.children = a.children);
        if (E5.zH) {
          var d;
          (d = a.props).idomKey || (d.idomKey = E5.Saa(b));
        }
        d = !1;
        if (c) {
          var e = nmb(c, a.props.idomKey);
          a.Cq = !0;
          var f = e.__instance;
        }
        if (!f) {
          d = !0;
          var h = a.props;
          if (knb(b)) {
            var l = new L5(h);
            l.C = b;
          } else l = new b(h);
          l.j = b;
          l.sM = { OP: l.state, WO: !1 };
          f = l;
          f.props = null;
          e && ((e.__instance = f), (f.el = e));
        }
        var m,
          n = null != (m = f.sM) ? m : { OP: f.state, WO: !1 },
          p = n.WO,
          q = n.OP;
        f.sM = void 0;
        b.c6 && (q = b.c6(a.props, q));
        c = !d && !p && !1;
        f.props = a.props;
        f.state = q;
        if (c) x5 = t5.lastChild;
        else {
          q = f;
          q5.kV(q);
          var r = q.bx(q.props);
          if (r) {
            q.props.idomKey && (r.props.idomKey = q.props.idomKey);
            var t = r;
          } else t = void 0;
          if (t) {
            t.Cq = a.Cq;
            var u = O5(t);
            a.Cq = t.Cq;
            if (!b.zy)
              if (u)
                (b.zy = u.tagName.toLowerCase()),
                  (u.__instance = f),
                  (f.el = u);
              else {
                var x;
                if (null == (x = f.Kz) ? 0 : x.length) {
                  var B;
                  null == (B = E5.JD) ||
                    B.call(
                      E5,
                      "unknown",
                      Error(
                        "A component used hooks, but failed to return a host element"
                      )
                    );
                }
              }
          }
          f.RC();
          q5.RC(f);
        }
      }
    } catch (H) {
      b = H;
      var F, G;
      null == (G = E5.JD) ||
        G.call(E5, null == (F = a.type) ? void 0 : F.zy, b);
    }
    a.Cq && (w5(null), (x5 = t5), (t5 = t5.parentNode), (a.Cq = !1));
  };
  E5.zH = !0;
  (function () {
    var a = C5;
    C5 = function (b) {
      null == a || a(b);
      nnb(b);
    };
  })();
  var Q5 = Ylb;
  var U5;
  E5.zO = wnb;
  U5 = ynb;
  E5.zO = wnb;
  tmb({
    QZ: {
      kV: function (a) {
        var b;
        (null == (b = a.Hy) ? 0 : b.length) && Fnb(a.Hy);
      },
      RC: function (a) {
        var b;
        (null == (b = a.Hy) ? 0 : b.length) && Fnb(a.Hy);
      },
      tH: function (a) {
        var b;
        (null == (b = a.Kz) ? 0 : b.length) && a.Kz.forEach(Enb);
      },
    },
  });
  var Lnb = [],
    Mnb = !1;
  g.w(Tnb, g.U);
  g.k = Tnb.prototype;
  g.k.hide = function () {
    this.N = !0;
    g.U.prototype.hide.call(this);
    Unb(this, !1);
  };
  g.k.show = function () {
    this.N = !1;
    g.U.prototype.show.call(this);
    Unb(this, !0);
  };
  g.k.isHidden = function () {
    return this.N;
  };
  g.k.P1 = function () {
    this.scrollTo(this.C - this.containerWidth);
  };
  g.k.Q1 = function () {
    this.scrollTo(this.C + this.containerWidth);
  };
  g.k.resize = function (a, b) {
    var c = this.api.U(),
      d = 16 / 9,
      e = 650 <= a.width,
      f = 480 > a.width || 290 > a.height,
      h = Math.min(this.suggestionData.length, this.j.length);
    if (150 >= Math.min(a.width, a.height) || 0 === h || !c.Id) this.hide();
    else {
      var l;
      if (e) {
        var m = (l = 28);
        this.B = 16;
      } else this.B = m = l = 8;
      if (f) {
        var n = 6;
        e = 14;
        var p = 12;
        f = 24;
        c = 12;
      } else (n = 8), (e = 18), (p = 16), (f = 36), (c = 16);
      a = a.width - (48 + l + m);
      l = Math.ceil(a / 150);
      l = Math.min(3, l);
      m = a / l - this.B;
      var q = Math.floor(m / d);
      b &&
        q + 100 > b &&
        50 < m &&
        ((q = Math.max(b, 50 / d)),
        (l = Math.ceil(a / (d * (q - 100) + this.B))),
        (m = a / l - this.B),
        (q = Math.floor(m / d)));
      50 > m || g.qU(this.api) ? this.hide() : this.show();
      for (b = 0; b < h; b++) {
        d = this.j[b];
        var r = d.Ga("ytp-suggestion-image");
        r.style.width = m + "px";
        r.style.height = q + "px";
        d.Ga("ytp-suggestion-title").style.width = m + "px";
        d.Ga("ytp-suggestion-author").style.width = m + "px";
        d = d.Ga("ytp-suggestion-duration");
        d.style.display = d && 100 > m ? "none" : "";
      }
      h = e + n + p + 4;
      this.K = h + c + (q - f) / 2;
      this.suggestions.element.style.height = q + h + "px";
      this.D = m;
      this.containerWidth = a;
      this.columns = l;
      this.C = 0;
      this.suggestions.element.scrollLeft = -0;
      Vnb(this);
    }
  };
  g.k.onVideoDataChange = function () {
    var a = this.api.getVideoData(),
      b = this.api.U();
    this.Y = a.Qd ? !1 : b.D;
    a.suggestions
      ? (this.suggestionData = g.Ys(a.suggestions, function (c) {
          return c && !c.playlistId;
        }))
      : (this.suggestionData.length = 0);
    Wnb(this);
    a.Qd
      ? this.title.update({
          title: g.lK("More videos from $DNI_RELATED_CHANNEL", {
            DNI_RELATED_CHANNEL: a.author,
          }),
        })
      : this.title.update({ title: "More videos on YouTube" });
  };
  g.k.scrollTo = function (a) {
    a = g.ne(
      a,
      this.containerWidth - this.suggestionData.length * (this.D + this.B),
      0
    );
    this.ma.start(this.C, a, 1e3);
    this.C = a;
    Vnb(this);
    Unb(this, !0);
  };
  g.w(Z5, g.VV);
  Z5.prototype.show = function () {
    g.VV.prototype.show.call(this);
    Ynb(this, this.api.ob().getPlayerSize());
  };
  Z5.prototype.resize = function (a) {
    g.VV.prototype.resize.call(this, a);
    this.j &&
      (Ynb(this, a),
      g.Mu(
        this.element,
        "related-on-error-overlay-visible",
        !this.j.isHidden()
      ));
  };
  Z5.prototype.B = function (a) {
    g.VV.prototype.B.call(this, a);
    var b = this.api.getVideoData();
    if (b.lC || b.playerErrorMessageRenderer)
      (a = b.lC)
        ? aob(this, a)
        : b.playerErrorMessageRenderer &&
          aob(this, b.playerErrorMessageRenderer);
    else {
      var c;
      a.qn &&
        (b.py
          ? Znb(b.py)
            ? (c = g.oG(b.py))
            : (c = g.WV(g.nG(b.py)))
          : (c = g.WV(a.qn)),
        this.le(c, "subreason"));
    }
  };
  g.w($5, g.U);
  $5.prototype.aj = function () {
    this.api.Sb().isCued() || (this.hide(), this.api.Wa(this.element, !1));
  };
  $5.prototype.j = function () {
    var a = this.api.getVideoData(),
      b = this.api.U(),
      c = this.api.getVideoData().Qd,
      d = b.Zd,
      e = !b.Id,
      f = this.ub.Nf(),
      h = g.kS(b);
    b = b.C;
    d || f || c || e || h || b || g.rU(this.api) || !a.videoId
      ? (this.hide(), this.api.Wa(this.element, !1))
      : ((a = bob(this)), this.updateValue("url", a), this.show());
  };
  $5.prototype.onClick = function (a) {
    this.api.L("web_player_log_click_before_generating_ve_conversion_params") &&
      this.api.rb(this.element);
    var b = bob(this);
    g.LU(b, this.api, a);
    this.api.L("web_player_log_click_before_generating_ve_conversion_params") ||
      this.api.rb(this.element);
  };
  $5.prototype.show = function () {
    this.api.Sb().isCued() &&
      (g.U.prototype.show.call(this),
      this.api.hk(this.element) && this.api.Wa(this.element, !0));
  };
  g.w(a6, g.U);
  a6.prototype.aj = function () {
    var a = this.api.Sb();
    !this.api.Sh() || (g.SG(a, 2) && g.nU(this.api)) || g.SG(a, 64)
      ? (this.api.Wa(this.j, !1),
        this.api.Wa(this.forwardButton, !1),
        this.hide())
      : (this.show(),
        this.api.Wa(this.j, !0),
        this.api.Wa(this.forwardButton, !0));
  };
  a6.prototype.B = function () {
    this.api.seekBy(-10 * this.api.getPlaybackRate(), void 0, void 0, 83);
    this.api.rb(this.j);
  };
  a6.prototype.C = function () {
    this.api.seekBy(10 * this.api.getPlaybackRate(), void 0, void 0, 82);
    this.api.rb(this.forwardButton);
  };
  g.w(b6, g.U);
  b6.prototype.C = function () {
    var a = this.api.Sb(),
      b = this.api.getVideoData();
    this.api.U().L("embeds_enable_muted_autoplay") &&
      b.mutedAutoplay &&
      (g.SG(a, 2) && !this.Fb
        ? (this.show(),
          this.j ||
            ((this.j = new g.TV(this.api)),
            g.L(this, this.j),
            this.j.Ea(this.D, 0),
            this.j.show()),
          (a = this.api.getVideoData()),
          this.updateValue("text", a.P0),
          g.Mu(this.element, "ytp-muted-autoplay-show-end-panel", !0),
          this.api.Wa(this.element, this.Fb),
          this.api.Qa("onMutedAutoplayEnds"))
        : this.hide());
  };
  b6.prototype.onClick = function () {
    if (!this.B) {
      this.j && (this.j.ya(), (this.j = null));
      g.Mu(this.api.getRootNode(), "ytp-muted-autoplay", !1);
      var a = this.api.getVideoData(),
        b = this.api.getCurrentTime();
      Wlb(a);
      this.api.loadVideoById(a.videoId, b);
      this.api.nA();
      this.api.rb(this.element);
      this.hide();
      this.B = !0;
    }
  };
  b6.prototype.onMutedAutoplayStarts = function () {
    this.B = !1;
    this.j && (this.j.ya(), (this.j = null));
  };
  g.w(c6, g.U);
  g.k = c6.prototype;
  g.k.x0 = function () {
    var a = this.api.getVideoData(),
      b = this.api.getVideoData(1).Qd,
      c = this.api.U();
    b = c.Zd && !g.SG(this.state, 2) && !(b && c.ma);
    a.mutedAutoplay || g.jG(this, b);
    this.api.Wa(this.element, b);
  };
  g.k.onStateChange = function (a) {
    this.Gc(a.state);
  };
  g.k.Gc = function (a) {
    this.state !== a && (this.state = a);
    this.x0();
  };
  g.k.onVideoDataChange = function () {
    var a = this.api.U();
    a.C && g.Iu(this.element, "ytp-no-hover");
    this.api.getVideoData().videoId && !a.C
      ? ((a = this.api.getVideoUrl(!0, !1, !1, !0)),
        this.updateValue("url", a),
        this.j || (this.j = this.Ta("click", this.onClick)))
      : this.j &&
        (this.updateValue("url", null), this.Pc(this.j), (this.j = null));
  };
  g.k.onClick = function (a) {
    this.api.L("web_player_log_click_before_generating_ve_conversion_params") &&
      this.api.rb(this.element);
    var b = this.api.getVideoUrl(!g.bH(a), !1, !0, !0);
    if (
      this.api.L("web_player_log_click_before_generating_ve_conversion_params")
    ) {
      var c = {};
      g.dU(this.api, "addEmbedsConversionTrackingParams", [c]);
      b = g.Ml(b, g.CP(c, "emb_yt_watermark"));
    }
    g.LU(b, this.api, a);
    this.api.L("web_player_log_click_before_generating_ve_conversion_params") ||
      this.api.rb(this.element);
  };
  g.k.Jb = function (a) {
    if (((a = 480 > a.width) && !this.B) || (!a && this.B)) {
      var b = new g.U(Xlb()),
        c = this.Ga("ytp-watermark");
      g.Mu(c, "ytp-watermark-small", a);
      g.qf(c);
      b.Ea(c);
      this.B = a;
    }
  };
  g.w(eob, g.U);
  g.k = eob.prototype;
  g.k.zL = function () {
    var a = this.api.Sb(),
      b = this.api.getVideoData();
    this.api.U().L("embeds_enable_muted_autoplay") &&
    b.mutedAutoplay &&
    !g.SG(a, 2)
      ? this.Fb ||
        (g.U.prototype.show.call(this), this.api.Wa(this.element, this.Fb))
      : this.hide();
  };
  g.k.f7 = function () {
    this.j && this.j.hide();
  };
  g.k.onAutoplayBlocked = function () {
    this.hide();
    Wlb(this.api.getVideoData());
  };
  g.k.onClick = function () {
    if (!this.B) {
      g.Mu(this.api.getRootNode(), "ytp-muted-autoplay", !1);
      var a = this.api.getVideoData(),
        b = this.api.getCurrentTime();
      Wlb(a);
      this.api.loadVideoById(a.videoId, b);
      this.api.nA();
      this.api.rb(this.element);
      this.api.Qa("onMutedAutoplayEnds");
      this.B = !0;
    }
  };
  g.k.onMutedAutoplayEnds = function () {
    this.watermark && (this.watermark.ya(), (this.watermark = null));
  };
  g.w(d6, g.U);
  d6.prototype.hide = function () {
    g.Ku(this.api.getRootNode(), "ytp-expand-pause-overlay");
    g.U.prototype.hide.call(this);
  };
  d6.prototype.G = function () {
    this.B = !0;
    g.Ku(this.api.getRootNode(), "ytp-expand-pause-overlay");
    g.rU(this.api) && this.api.Wa(this.element, !1);
    this.expandButton.focus();
  };
  d6.prototype.K = function () {
    this.B = !1;
    g.Iu(this.api.getRootNode(), "ytp-expand-pause-overlay");
    g.rU(this.api) && this.api.Wa(this.element, !0);
    this.focus();
  };
  d6.prototype.Ra = function () {
    var a = this.api.Sb();
    g.SG(a, 1) ||
      g.SG(a, 16) ||
      g.SG(a, 32) ||
      (!g.SG(a, 4) || g.SG(a, 2) || g.SG(a, 1024)
        ? (this.B || this.api.Wa(this.element, !1), this.D.hide())
        : this.j.ND() &&
          (this.B ||
            (g.Iu(this.api.getRootNode(), "ytp-expand-pause-overlay"),
            g.u2(this.j),
            this.j.show(),
            this.api.Wa(this.element, !0)),
          this.D.show()));
  };
  g.w(e6, g.U);
  e6.prototype.show = function () {
    g.U.prototype.show.call(this);
    this.api.Wa(this.element, !0);
  };
  e6.prototype.ya = function () {
    f6(this);
    g.U.prototype.ya.call(this);
  };
  g.w(iob, g.zV);
  g.k = iob.prototype;
  g.k.Gl = function () {
    return !1;
  };
  g.k.create = function () {
    var a = this.J.U(),
      b = g.gU(this.J),
      c,
      d = null == (c = this.J.getVideoData()) ? void 0 : c.clientPlaybackNonce;
    d && g.ZH({ clientPlaybackNonce: d });
    kob(this);
    a.L("embeds_enable_muted_autoplay") &&
      ((this.Z = new eob(this.J)),
      g.L(this, this.Z),
      g.uU(this.J, this.Z.element, 4),
      (this.ra = new b6(this.J)),
      g.L(this, this.ra),
      g.uU(this.J, this.ra.element, 4));
    a.Zd &&
      ((this.watermark = new c6(this.J)),
      g.L(this, this.watermark),
      g.uU(this.J, this.watermark.element, 8));
    this.K = new $5(this.J, b);
    g.L(this, this.K);
    g.uU(this.J, this.K.element, 8);
    g.eU(this.J.app) && (this.onMutedAutoplayStarts(), this.K.hide());
    a.B &&
      ((this.ma = new a6(this.J)),
      g.L(this, this.ma),
      g.uU(this.J, this.ma.element, 4));
    this.j.T(this.J, "appresize", this.Jb);
    this.j.T(this.J, "presentingplayerstatechange", this.aj);
    this.j.T(this.J, "videodatachange", this.onVideoDataChange);
    this.j.T(this.J, "videoplayerreset", this.Lw);
    this.j.T(this.J, "onMutedAutoplayStarts", this.onMutedAutoplayStarts);
    this.j.T(this.J, "onAdStart", this.onAdStart);
    this.j.T(this.J, "onAdComplete", this.onAdComplete);
    this.j.T(this.J, "onAdSkip", this.onAdSkip);
    this.j.T(this.J, "onAdStateChange", this.onAdStateChange);
    if ((this.N = g.FC(g.lS(a))))
      (this.countdownTimer = new e6(this.J)),
        g.L(this, this.countdownTimer),
        g.uU(this.J, this.countdownTimer.element, 4),
        this.countdownTimer.hide(),
        this.j.T(this.J, g.fK("embeds"), this.onCueRangeEnter),
        this.j.T(this.J, g.gK("embeds"), this.onCueRangeExit);
    this.zc(this.J.Sb());
    this.player.Gg("embed");
    var e, f;
    (null == (e = this.J.U().webPlayerContextConfig)
      ? 0
      : null == (f = e.embedsHostFlags)
      ? 0
      : f.allowOverridingVisitorDataPlayerVars) &&
      (a = g.IB("IDENTITY_MEMENTO")) &&
      this.J.Qa("onMementoChange", a);
  };
  g.k.onCueRangeEnter = function (a) {
    "countdown timer" === a.getId() &&
      this.countdownTimer &&
      (this.countdownTimer.show(), gob(this.countdownTimer));
  };
  g.k.onCueRangeExit = function (a) {
    "countdown timer" === a.getId() &&
      this.countdownTimer &&
      (f6(this.countdownTimer), this.countdownTimer.hide());
  };
  g.k.Jb = function () {
    var a = this.J.ob().getPlayerSize();
    this.Jg && this.Jg.resize(a);
  };
  g.k.Lw = function () {
    hob(this);
  };
  g.k.aj = function (a) {
    this.zc(a.state);
  };
  g.k.zc = function (a) {
    g.SG(a, 128)
      ? (this.Jg ||
          ((this.Jg = new Z5(this.J)),
          g.L(this, this.Jg),
          g.uU(this.J, this.Jg.element, 4)),
        this.Jg.B(a.uh),
        this.Jg.show(),
        g.Iu(this.J.getRootNode(), "ytp-embed-error"))
      : this.Jg &&
        (this.Jg.dispose(),
        (this.Jg = null),
        g.Ku(this.J.getRootNode(), "ytp-embed-error"));
    if (this.countdownTimer && this.countdownTimer.j)
      if (g.SG(a, 64)) this.countdownTimer.hide(), f6(this.countdownTimer);
      else if (a.isPaused()) {
        var b = this.countdownTimer;
        b.B || ((b.B = !0), (b.G = (0, g.FD)()));
      } else
        a.isPlaying() &&
          this.countdownTimer.B &&
          ((b = this.countdownTimer),
          b.B && ((b.D += (0, g.FD)() - b.G), (b.B = !1), fob(b)));
    job(this, a);
  };
  g.k.onMutedAutoplayStarts = function () {
    this.J.getVideoData().mutedAutoplay &&
      this.Z &&
      g.Mu(this.J.getRootNode(), "ytp-muted-autoplay", !0);
  };
  g.k.onVideoDataChange = function (a, b) {
    var c = this.gE !== b.videoId;
    a = !c && "dataloaded" === a;
    var d = { isShortsModeEnabled: !!g.rU(this.J) };
    g.qD("embedsVideoDataDidChange", {
      clientPlaybackNonce: b.clientPlaybackNonce,
      isReload: a,
      runtimeEnabledFeatures: d,
    });
    c &&
      ((this.gE = b.videoId),
      this.countdownTimer &&
        (this.countdownTimer.show(), this.countdownTimer.hide()),
      this.N &&
        (this.J.lf("embeds"),
        b.isAd() ||
          5 > b.limitedPlaybackDurationInSeconds ||
          g.qU(this.J) ||
          ((b = Math.max(
            1e3 * (b.startSeconds + b.limitedPlaybackDurationInSeconds - 5),
            0
          )),
          (b = new g.eK(b, b + 5e3, {
            id: "countdown timer",
            namespace: "embeds",
          })),
          this.J.yf([b]))),
      hob(this),
      kob(this));
    this.J.U().C && this.C && this.C.detach();
  };
  g.k.onAdStart = function () {
    g6(this, "EMBEDS_AD_EVENT_TYPE_AD_STARTED");
  };
  g.k.onAdComplete = function () {
    g6(this, "EMBEDS_AD_EVENT_TYPE_AD_COMPLETED");
  };
  g.k.onAdSkip = function () {
    g6(this, "EMBEDS_AD_EVENT_TYPE_AD_SKIPPED");
  };
  g.k.onAdStateChange = function (a) {
    2 === a && g6(this, "EMBEDS_AD_EVENT_TYPE_AD_PAUSED");
  };
  g.yV("embed", iob);
})(_yt_player);
